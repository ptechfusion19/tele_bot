...
async def cmd_start(message: Message):
    kb = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text='/cancel'), KeyboardButton(text='/google')],
            [KeyboardButton(text='/github'), KeyboardButton(text='/remove')],
        ],
...
async def cmd_echo(message: Message):
    ...
    if text:
        ...
        await message.answer("Please provide text after /echo.")

@dp.message(Command('google'))
async def cmd_google(message: Message):
    await message.answer("🌐 [Google](https://www.google.com)")

@dp.message(Command('github'))
async def cmd_github(message: Message):
    await message.answer("💻 [GitHub](https://github.com)")
...
async def begin(message: Message, state: FSMContext):
    await message.answer("📝 Great! What is your *name*?")
    await state.set_state(Form.name)

@dp.message(Form.name)
async def name_input(message: Message, state: FSMContext):
    await state.update_data(name=message.text)
    await message.answer("Nice to meet you! What is your *email*?")
...
async def email_input(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"📧 You said your email is: *{message.text}*\nIs this correct? (Yes/No)"
    )
...
async def confirm_input(message: Message, state: FSMContext):
    ...
    if text == "yes":
        ...
        await message.answer(
            f"✅ Thank you *{data['name']}*! Your info has been recorded.",
            reply_markup=ReplyKeyboardRemove()
        )
        await state.clear()
    elif text == "no":
